...
class GeneratorResNet_2(nn.Module):
    def __init__(self, input_shape):  # (input_shape_A 和 input_shape_B = (3, 256, 256),
        ...
        in_features = 256 + 64 + 64  # in_features = 256
        ...
        in_features = 128 + 32 + 32  # in_features = 128  256+128=384
        ...
        in_features = 64 + 16 + 16  # in_features = 64
        ...
        in_features = 32 + 8 + 8  # in_features = 32
    ...
    def forward(self, x1, x2, x3):  # 输入(1, 3, 256, 256) (1, 3, 256, 256) (batchsize, channels, height, width)
        # x1 为 中间结果
        # x2 为 原可见光
        # x3 为 原红外
        ...
        x2_repeat_2 = x2.repeat(1, 64, 1, 1)
        x3_repeat_2 = x3.repeat(1, 64, 1, 1)
        input_2 = torch.cat((result_1, x2_repeat_2), dim=1)  # 此时第二步的输入通道等于 256+64
        input_2 = torch.cat((input_2, x3_repeat_2), dim=1)  # 此时第二步的输入通道等于 256+64+64 = 384
        result_2 = self.model_2(input_2)

        x2_repeat_3 = x2.repeat(1, 32, 1, 1)
        x3_repeat_3 = x3.repeat(1, 32, 1, 1)
        input_3 = torch.cat((result_2, x2_repeat_3), dim=1)  # 此时第三步的输入通道等于 128+32
        input_3 = torch.cat((input_3, x3_repeat_3), dim=1)  # 此时第三步的输入通道等于 128+32+32 = 192
        result_3 = self.model_3(input_3)

        x2_repeat_4 = x2.repeat(1, 16, 1, 1)
        x3_repeat_4 = x3.repeat(1, 16, 1, 1)
        input_4 = torch.cat((result_3, x2_repeat_4), dim=1)  # 此时第四步的输入通道等于 64+16
        input_4 = torch.cat((input_4, x3_repeat_4), dim=1)  # 此时第四步的输入通道等于 64+16+16 = 96
        result_4 = self.model_4(input_4)

        x2_repeat_5 = x2.repeat(1, 8, 1, 1)
        x3_repeat_5 = x3.repeat(1, 8, 1, 1)
        input_5 = torch.cat((result_4, x2_repeat_5), dim=1)  # 此时第五步的输入通道等于 32+8
        input_5 = torch.cat((input_5, x3_repeat_5), dim=1)  # 此时第五步的输入通道等于 32+8+8 = 48
...
# test_G = GeneratorResNet_2((1, 256, 256)).cuda()
...
class Discriminator_2(nn.Module):
    def __init__(self, input_shape):
        ...
        self.output_shape_1 = (1, height // 2 ** 4, width // 2 ** 4)  # output_shape_1 = (1, 16, 16)
        self.output_shape_2 = (1, height // 4, width // 4)  # # output_shape_2 = (1, 64, 64)
        '''!!!!!!!!!!!!!!!!!!!!!!!!!!!!输入图像是3维的，输出图像是1维16*16的概率矩阵!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        patchGAN有关，这是判别器。不是生成器！！！output不是融合图像，而是判别器的输出结果（矩阵）      '''

        def discriminator_block(in_filters, out_filters, kernel_size=4, stride=2, padding=1, normalize=True):  # 鉴别器块儿
            """Returns downsampling layers of each discriminator block"""
            layers = [
                nn.Conv2d(in_filters, out_filters, kernel_size=kernel_size, stride=stride,
                          padding=padding)]  # layer += [conv + norm + relu]
        ...
        self.model_2 = nn.Sequential(
            *discriminator_block(channels, 32, kernel_size=3, stride=2, normalize=False),
            *discriminator_block(32, 64, kernel_size=3, stride=2),
            *discriminator_block(64, 128, kernel_size=3, stride=1),
            *discriminator_block(128, 256, kernel_size=3, stride=1),
            nn.ZeroPad2d((1, 0, 1, 0)),
            nn.Conv2d(256, 1, kernel_size=4, padding=1)
        )

    def forward(self, img):  # 输入(1, 3, 256, 256)
        result_1 = self.model_1(img)
        result_2 = self.model_2(img)  # 输出(1, 1, 16, 16)  输出结果为(batchsize=1, channels=1, H=16, W=16)的矩阵，表示patchGAN
        # print("result_1.shape:", result_1.shape)  # [2, 1, 16, 16]
        # print("result_2.shape:", result_2.shape)  # [2, 1, 64, 64]
        return result_1, result_2


# test_D = Discriminator_2((1, 256, 256)).cuda()
